...
runLoop:
	for {
		...
		// 	fmt.Println("Time: ",float64(durationTmp.Nanoseconds())/1000000.0)
		// }
...
func (s *session) handleStreamFrame(frame *wire.StreamFrame) error {
	...
	if frame.FinBit {
		...
		for pathID, pth := range s.paths {
			...
			utils.Infof("Path %x: sent %d retrans %d lost %d; rcv %d; loss rate: %f", pathID, sntPkts, sntRetrans, sntLost, rcvPkts, float64(sntLost)/float64(sntPkts))
		}
		s.pathsLock.RUnlock()
	}
	...
	if err == nil && pth.pathID != protocol.InitialPathID && s.scheduler.SchedulerName == "multiclients" {
		if _, ok := s.scheduler.list_State[State{pth.pathID, pth.lastRcvdPacketNumber}]; ok {
			if oldLost == newLost {
				...
				s.scheduler.GetStateAndRewardMultiClientsRetrans(s, pth)
			}
...
func (s *session) handleCloseError(closeErr closeError) error {
	...
	// 	multiclients.CloseListSessions[uint64(s.connectionID)] = 0
	// }
