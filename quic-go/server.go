...
func (s *server) handlePacket(rcvRawPacket *receivedRawPacket) error {
	...
	if !ok {
		...
		// 		multiclients.S1[uint64(hdr.ConnectionID)] = &multiclients.StateMulti{}
		// 		fmt.Println("MultiLen: ", len(multiclients.S1))
		...
		if time.Time.IsZero(multiclients.ServerCreationTime) {
			now := time.Now()
			multiclients.ServerCreationTime = now
		}
...
func (s *server) removeConnection(id protocol.ConnectionID) {
	...
	s.sessions[id] = nil
	s.sessionsMutex.Unlock()
	var connID string = strconv.FormatUint(uint64(id), 10)
