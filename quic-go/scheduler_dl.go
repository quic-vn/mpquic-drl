...
func (sch *scheduler) GetStateAndRewardDQN(s *session, pth *path) {
	...
	reWard[firstPath] = (1-s.scheduler.Alpha)*float64(CWNDlevel[firstPath]) - s.scheduler.Alpha*float64(NormalizeTimes(lRTT[firstPath]))
	reWard[secondPath] = (1-s.scheduler.Alpha)*float64(CWNDlevel[secondPath]) - s.scheduler.Alpha*float64(NormalizeTimes(lRTT[secondPath]))

	// var action int8
	// if pth.pathID == 1 {
	// 	action = 0
	// } else if pth.pathID == 3 {
	// 	action = 1
	// }
	//Trang thai cu
