...
def configClient(sta, id):
    ...
    sta.cmd("ip route add default nexthop via 192.168.2.2 dev wlan0 weight 1")
...
def topology(args, server_cmd, client_cmd):
    ...
    for i in range(3, 3 + int(args.clt)):
        if args.mdl == 'mobi':
            sta = net.addStation('sta%d' % i, wlans=1, mac='00:00:00:00:01:%02d' % i)
            stations.append(sta)
        elif args.mdl == 'none': #same place
            x = 50
            y = 50
            sta = net.addStation('sta%d' % i, wlans=1, mac='00:00:00:00:01:%02d' % i, position='%d,%d,0' % (x, y))
            stations.append(sta)
